...
| RK-19 | Topology extraction changes graph edges, edge order, initialization, parameter order, or format-v1 resume. | invalid compatibility or comparison evidence | pre-refactor seeded golden graphs and fixed CPU snapshot, strict v1 tests, independent QA | active |
| RK-20 | Control graphs or seeds are selected after inspecting structure or performance. | selection bias and invalid H-03 attribution | one config/one graph, fixed attempts and overlap rules, deterministic failure, no performance run in Stage 05 | active |
| RK-21 | Checkpoint config/provenance names one topology while same-shaped graph buffers contain another. | silent resume/evaluation mismatch | regenerate expected graph, exact buffer/hash verification, cross-topology resume rejection | active |
| RK-22 | In-place resume overwrites the pre-resume checkpoint named by provenance. | exact historical replay may lack its original input | record the exact invocation; retain/archive pre-resume checkpoints for evidence runs that require later exact replay | active |
